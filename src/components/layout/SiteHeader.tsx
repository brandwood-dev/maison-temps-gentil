...
export function SiteHeader({ cartCount = 0 }: { cartCount?: number }) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-[color:var(--color-border)] bg-[color:var(--color-background)]/95 backdrop-blur supports-[backdrop-filter]:bg-[color:var(--color-background)]/80">
      {/* Mobile */}
      <div className="container-page flex h-14 items-center justify-between md:hidden">
        <button
          type="button"
        ...
        <a href="/" aria-label="La Maison des Montres — Accueil" className="inline-flex">
          <Logo variant="dark" height={28} priority />
        </a>
            ...
            <ul className="flex items-center justify-center gap-6 lg:gap-8">
              {NAV_LINKS.map((l) => (
                <li key={l.href}>
                  <a
                    href={l.href}
                    className="text-[13px] font-medium tracking-wide text-[color:var(--color-foreground)] transition-colors hover:text-[color:var(--color-gold)]"
                  >
                    {l.label}
                  </a>
                </li>
